...
当前版本：`2.9.0`
`2.9.0` 汇总了 `2.8.1` 以来这一轮体验升级，重点把插件补成了“更稳的屏幕伙伴 + 更像 KeyStats / Work_Review 的本地工作回顾工具”：安装更稳、活动轨迹更清楚、离开自动挂起更自然、日记也更不容易断更。现在也支持单独开启独立活动轨迹采集，即使没有启动自动观察，也能按固定间隔记录当前窗口、应用和站点轨迹；自动观察运行中则继续优先使用识屏轨迹，避免两套记录互相打架。
...
- 如果你不需要麦克风监听，直接升级到 `2.9.0` 后重新上传插件即可。
...
这是旧版本运行状态接口里的已知问题，`2.9.0` 已包含这项修复。
