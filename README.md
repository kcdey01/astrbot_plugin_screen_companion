...
astrbot_plugin_screen_companion 是面向 AstrBot 的屏幕伙伴插件。它能让bot随机地监视你的屏幕，并根据场景与用户进行互动。插件能监控音量、电脑状态以及你打开的窗口并进行合适的互动，会记录你的电脑使用情况，且会根据每天的观察记录完成日记。此外，插件还拥有一个完善的webUI界面。灵感来源于妹居物语，适配live2D桌宠。
...
`2.9.1` 在 `2.9.0` 的基础上继续补强了这轮体验升级，补上了日记单独 / 批量删除、今日本地输入统计命令和窗口陪伴快捷添加，并对部分文本进行了优化。
