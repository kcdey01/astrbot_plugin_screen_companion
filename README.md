...
让bot能够监视你的屏幕和电脑使用情况，并根据场景与用户进行互动。插件能监控音量、电脑状态以及你打开的窗口并进行合适的互动，且会根据每天的观察记录完成日记。此外，插件还拥有一个完善的webUI界面。灵感来源于妹居物语，适配live2D桌宠。
...
当前版本：`2.9.9`
`2.9.9` 主要包含内置工具调用链路更新：bot 现在可以自行决定何时调用 `screen_peek` 和 `screen_usage_context`，并补强了电脑使用情况摘要的场景分布与常用应用聚合。
